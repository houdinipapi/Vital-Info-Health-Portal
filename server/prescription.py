...
def show_prescription_details(list_of_prescriptions):
    ...
    if len(list_of_prescriptions) == 0:
        ...
        series = pd.Series(data=prescription_details,
                           index=prescription_titles)
        st.write(series)
    else:
        prescription_details = []
        for prescription in list_of_prescriptions:
            prescription_details.append([x for x in prescription])
        df = pd.DataFrame(data=prescription_details,
                          columns=prescription_titles)
        st.write(df)
...
class Prescription:
    ...
    def add_prescription(self):
        ...
        self.medicine_1_dosage_description = st.text_area(
            'Medicine 1 dosage and description')
        med_2_name = st.text_input('Medicine 2 name (if any)')
        self.medicine_2_name = (
            lambda name: None if name == '' else name)(med_2_name)
        med_2_dose_desc = st.text_area('Medicine 2 dosage and description')
        self.medicine_2_dosage_description = (
            lambda dose_desc: None if dose_desc == '' else dose_desc)(med_2_dose_desc)
        med_3_name = st.text_input('Medicine 3 name (if any)')
        self.medicine_3_name = (
            lambda name: None if name == '' else name)(med_3_name)
        med_3_dose_desc = st.text_area('Medicine 3 dosage and description')
        self.medicine_3_dosage_description = (
            lambda dose_desc: None if dose_desc == '' else dose_desc)(med_3_dose_desc)
    ...
    def update_prescription(self):
        id = st.text_input(
            'Enter Prescription ID of the prescription to be updated')
        if id == '':
            st.empty()
    ...
    def delete_prescription(self):
        id = st.text_input(
            'Enter Prescription ID of the prescription to be deleted')
        if id == '':
            st.empty()
        elif not verify_prescription_id(id):
            st.error('Invalid Prescription ID')
    ...
    def prescriptions_by_patient(self):
        ...
        if patient_id == '':
            ...
            with conn:
                ...
                st.write('Here is the prescription record of',
